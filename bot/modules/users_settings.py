...
example_dict = {'prefix':'1. <code>@your_channel_username or Anything</code>', 
                ...
                'suffix':'1. <code>~ KC</code>\n2. <code>~ @channelname</code>', 
                'msuffix':'1. <code>~ KC</code>\n2. <code>~ @channelname</code>', 
...
├ ▫️ UserLog : <b>{userlog}</b>
├ ▫️ Prefix : <b>{escape(prefix)}</b>
├ ▫️ Suffix : <b>{suffix}</b>
├ ▫️ Caption : <b>{escape(caption)}</b>
├ ▫️ CapFont : {cfont}
├ ▫️ Leech Split Size : <b>{lsplit}</b>
├ ▫️ Equal Split : <b>{esplits}</b>
├ ▫️ Daily Leech : <b>{dailyll} / {dailytlle} per day</b>
├ ▫️ Remname : <code>{escape(remname)}</code>
'''
    if uplan == "Paid User" and key:
        ex_date = user_dict.get('expiry_date', False)
        if not ex_date: ex_date = 'Not Specified'
        text += f"├ ▫️ User Plan : <b>{uplan}</b>\n"
        text += f"╰ ▫️ Expiry Date : <b>{ex_date}</b>"
    elif key: text += f"╰ ▫️ User Plan : <b>{uplan}</b>"
    return text, button
